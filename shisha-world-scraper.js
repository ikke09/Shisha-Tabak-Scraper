...
const loadPageInfos = async () => {
    try {
        ...
        const producerItems = $('div.filter-panel:contains("Hersteller")').find('label.filter-panel--label');
...
const loadTobaccoDetails = async (tobacco) => {
    try {
        ...
        tobacco.ean = $('td.product--properties-label:contains("EAN")').next().text();
        ...
        const itemsToFindTobaccoName = [tobacco.producer.name, amount+unit, 'Tabak', 'Tobacco', '-', ',', ';'];
        const replacRegex = new RegExp(itemsToFindTobaccoName.join('|'), 'gi');
        tobacco.name = title.replaceAll(replacRegex, '').trim();

        return tobacco;
    } catch (error) {
        console.error(error.message || `Failed to load Details from ${tobacco.source}`);
        tobacco.error = {
